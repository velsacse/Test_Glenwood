 ...
 * @version $Revision$
 */
public class MobileMvpModule extends AbstractGinModule {
  ...
  protected void configure() {
    ...
    bind(PlaceHistoryMapper.class).to(MobileAppPlaceHistoryMapper.class).in(Singleton.class);
    
    /// ActivityMapper maps the place to a new activity instance.
    
    // You should have one activity mapper for each display area.
    bind(ActivityMapper.class).to(MobileMainActivityMapper.class).in(Singleton.class);
  ...
  public PlaceHistoryHandler getHistoryHandler(PlaceController placeController,
    ...
    historyHandler.register(placeController, eventBus, new AVPPlace("Hello Mobile"));
