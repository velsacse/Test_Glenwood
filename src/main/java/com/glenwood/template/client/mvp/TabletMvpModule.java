 ...
 * @version $Revision$
 */
public class TabletMvpModule extends AbstractGinModule {
  ...
  protected void configure() {
    ...
    bind(PlaceHistoryMapper.class).to(TabletAppPlaceHistoryMapper.class).in(Singleton.class);
    
    /// ActivityMapper maps the place to a new activity instance.
    
    // You should have one activity mapper for each display area.
    bind(ActivityMapper.class).to(TabletMainActivityMapper.class).in(Singleton.class);
  ...
  public PlaceHistoryHandler getHistoryHandler(PlaceController placeController,
    ...
    historyHandler.register(placeController, eventBus, new AVPPlace("Hello Tablet"));
